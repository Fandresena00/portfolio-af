    ...
    <div className="overflow-hidden rounded-lg border border-[rgb(var(--border))] bg-[rgb(var(--surface))]">
      ...
      <div className="p-6">
        <div className="relative flex items-center justify-between px-2 pt-1">
          <span className="absolute left-9 right-9 top-6 h-px bg-[rgb(var(--border))]" />
          {status !== "idle" && (
            <motion.span
              initial={{ left: "6%" }}
              animate={{ left: status === "sending" ? ["6%", "94%", "6%"] : "6%" }}
              transition={{ duration: 1.4, ease: "easeInOut" }}
              className="absolute top-[22px] h-2 w-2 rounded-full bg-[rgb(var(--blue))] shadow-[0_0_8px_rgb(var(--blue)/0.8)]"
            />
          )}
          {["Client", "API", "Database"].map((node, i) => (
            <div key={node} className="z-10 flex flex-col items-center gap-2">
              <span
                className={`flex h-10 w-10 items-center justify-center rounded-md border font-[family-name:var(--font-mono)] text-[10px] transition-colors ${
                  status !== "idle" ? "border-[rgb(var(--blue)/60%)] text-[rgb(var(--blue))]" : "border-[rgb(var(--border))] text-[rgb(var(--muted))]"
                }`}
              >
                {i === 0 ? "C" : i === 1 ? "API" : "DB"}
              </span>
              <span className="font-[family-name:var(--font-mono)] text-[10px] text-[rgb(var(--muted))]">{node}</span>
            </div>
          ))}
        </div>

        <div className="mt-6 min-h-[80px] rounded-md border border-[rgb(var(--border))] bg-[rgb(var(--panel))] p-3 font-[family-name:var(--font-mono)] text-[12px] leading-relaxed text-[rgb(var(--muted))]">
          <AnimatePresence mode="wait">
            {status === "idle" && (
              <motion.p key="idle" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                <span className="text-[rgb(var(--blue))]">GET</span> /api/projects/status
              </motion.p>
            )}
            {status === "sending" && (
              <motion.p key="sending" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                Waiting for response<span className="animate-pulse">…</span>
              </motion.p>
            )}
            {status === "done" && (
              <motion.pre
...
{`{
  ...
  "uptime": "ok"
        ...
        <div className="mt-4 flex items-center justify-between">
          ...
          >
            {status === "done" ? "Replay" : status === "sending" ? "Sending…" : "Send request"}
          </button>
          {status === "done" && (
